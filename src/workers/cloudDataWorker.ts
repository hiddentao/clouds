...
export interface PixelProperty {
  nx: number; ny: number; density: number; isEdge: boolean; edgeDistance: number;
  shadowFactor: number; brightness: number; alpha: number;
...
function _calculatePixelColorLogic(
  pixelInfo: { density: number; isEdge: boolean; edgeDistance: number; shadowFactor: number; brightness: number; alpha?: number }, // alpha is optional as it's not used for color calculation itself
  ...
  if (skyGradient) {
    ...
    if (isEdge || edgeDistance < 0.1) {
      ...
      finalColor = [ baseColor[0]*(1-mixFactor)+highlightColor[0]*mixFactor, baseColor[1]*(1-mixFactor)+highlightColor[1]*mixFactor, baseColor[2]*(1-mixFactor)+highlightColor[2]*mixFactor ];
    } else {
      ...
      if (shadowMix > 0) { const strength = shadowMix*0.6; finalColor = [ finalColor[0]*(1-strength)+shadowColor[0]*strength, finalColor[1]*(1-strength)+shadowColor[1]*strength, finalColor[2]*(1-strength)+shadowColor[2]*strength ]; }
      if (lightMix > 0) { const strength = lightMix*0.4; finalColor = [ finalColor[0]*(1-strength)+highlightColor[0]*strength, finalColor[1]*(1-strength)+highlightColor[1]*strength, finalColor[2]*(1-strength)+highlightColor[2]*strength ]; }
    }
    finalColor = [ Math.min(1.0, finalColor[0]*brightness), Math.min(1.0, finalColor[1]*brightness), Math.min(1.0, finalColor[2]*brightness) ];
    const r = Math.round(finalColor[0]*255); const g = Math.round(finalColor[1]*255); const b = Math.round(finalColor[2]*255);
    return (r << 16) | (g << 8) | b;
  }
  const gray = Math.floor(brightness*255); return (gray<<16)|(gray<<8)|gray;
}
...
const SIZE_RANGES: Record<CloudTypeKey, {w:{min:number;max:number};h:{min:number;max:number}}> = {
    wispy:{w:{min:400,max:800},h:{min:100,max:200}},scattered:{w:{min:350,max:700},h:{min:150,max:280}},
    puffy:{w:{min:500,max:1200},h:{min:200,max:450}},dense:{w:{min:600,max:1400},h:{min:250,max:500}},
    default:{w:{min:400,max:900},h:{min:180,max:350}}
};
const SPEED_RANGES: Record<CloudTypeKey, {min:number;max:number}> = {
    wispy:{min:0.8,max:1.5},scattered:{min:0.6,max:1.2},puffy:{min:0.2,max:0.6},dense:{min:0.3,max:0.8},default:{min:0.3,max:0.8}
};

function _getDensityForType(type: CloudTypeKey, seed: number): number {
    switch(type) {
        case 'wispy': return 0.4 + _simpleRandom(seed) * 0.4;
        case 'scattered': return 0.5 + _simpleRandom(seed) * 0.4;
        case 'puffy': return 0.7 + _simpleRandom(seed) * 0.3;
        default: return 0.6 + _simpleRandom(seed) * 0.3;
    }
}

function _getLayersForType(type: CloudTypeKey, seed: number): number {
    switch(type) {
        case 'wispy': return 1 + Math.floor(_simpleRandom(seed) * 2);
        case 'scattered': return 2 + Math.floor(_simpleRandom(seed) * 2);
        case 'puffy': return 3 + Math.floor(_simpleRandom(seed) * 3);
        default: return 2 + Math.floor(_simpleRandom(seed) * 3);
    }
}

function _calculateCloudDensity(
    nxLocal: number, nyLocal: number,
    fragmentData: CloudFragmentData, pNoise: PerlinNoise
): number {
    let dens = 0;
    ...
    switch (fragmentData.type) {
        case 'wispy': {
            ...
            const t1 = Math.max(0, 1.0 - Math.abs(nyLocal + 0.3 + tendrilOffset1) * (3.0 * turbulenceScale)) * Math.max(0, 1.0 - Math.abs(nxLocal) * (0.8 * complexityFactor));
            const t2 = Math.max(0, 1.0 - Math.abs(nyLocal - 0.2 + tendrilOffset2) * (4.0 * turbulenceScale)) * Math.max(0, 1.0 - Math.abs(nxLocal + 0.3) * (1.2 * complexityFactor));
            const t3 = Math.max(0, 1.0 - Math.abs(nyLocal + tendrilOffset3) * (2.5 * turbulenceScale)) * Math.max(0, 1.0 - Math.abs(nxLocal - 0.4) * (1.0 * complexityFactor));
        ...
        case 'puffy': {
            ...
            for (let i = 0; i < numMasses; i++) {
                const offsetX = _simpleRandom(fragmentData.noiseOffset + i * 10) * (0.6 * turbulenceScale) - 0.3 * turbulenceScale;
                const offsetY = _simpleRandom(fragmentData.noiseOffset + i * 20) * (0.4 * turbulenceScale) - 0.2 * turbulenceScale;
                const massRadius = (0.25 + _simpleRandom(fragmentData.noiseOffset + i * 30) * 0.35) * complexityFactor;
        ...
        case 'scattered': {
            ...
            for (let i = 0; i < numPatches; i++) {
                const offsetX = _simpleRandom(fragmentData.noiseOffset + i * 15) * (1.6 * turbulenceScale) - 0.8 * turbulenceScale;
                const offsetY = _simpleRandom(fragmentData.noiseOffset + i * 25) * (1.0 * turbulenceScale) - 0.5 * turbulenceScale;
                const patchSize = (0.15 + _simpleRandom(fragmentData.noiseOffset + i * 35) * 0.3) * complexityFactor;
                const distance = Math.sqrt((nxLocal - offsetX) ** 2 + (nyLocal - offsetY) ** 2);
        ...
        case 'dense': {
            ...
            for (let i = 0; i < numBulges; i++) {
                ...
                const bulgeSize = 0.8 + _simpleRandom(fragmentData.noiseOffset + i * 60) * (0.6 * complexityFactor);
                const bulgeStrength = 0.7 + _simpleRandom(fragmentData.noiseOffset + i * 70) * 0.2;
                const bulgeDensity = Math.max(0, 1.0 - Math.sqrt((nxLocal - bulgeX) ** 2 + (nyLocal - bulgeY) ** 2) * bulgeSize);
    ...
    if (fragmentData.type === 'dense') { noiseIntensity = 0.05 + fragmentData.turbulence * 0.03; }
    else if (fragmentData.type === 'puffy') { noiseIntensity = 0.08 + fragmentData.turbulence * 0.05; }
    else { noiseIntensity = 0.15 + fragmentData.turbulence * 0.1; }
...
function _calculateOrganicBoundary(
    ...
    switch (fragmentData.type) {
        case 'wispy': {
            ...
            const tendrilNoise = pNoise.noise(nxLocal * 3 + fragmentData.noiseOffset * 0.01, nyLocal * 4) * 0.3;
        ...
        case 'scattered': {
            const patchNoise = pNoise.noise(nxLocal * 4 + fragmentData.noiseOffset * 0.01, nyLocal * 4) * 0.4;
            const baseNoise = pNoise.noise(nxLocal * 2, nyLocal * 2) * 0.2;
            boundary = 0.7 + patchNoise + baseNoise;
            break;
        }
    }
    const organicNoiseVal = pNoise.noise(nxLocal * 2.5 + fragmentData.noiseOffset * 0.005, nyLocal * 2.5) * 0.1;
...
const cloudDataGenerator = {
  generateFullCloudData(
    ...
    const fragmentTypesArray: Array<'wispy' | 'puffy' | 'dense' | 'scattered'> = ['wispy', 'puffy', 'dense', 'scattered'];
    ...
    const depthLayer = layerKeys[Math.floor(_simpleRandom(noiseOffsetForLayerSelection) * layerKeys.length)];
    ...
    const scale = layerConfig.scaleRange.min + _simpleRandom(noiseOffset + 1) * (layerConfig.scaleRange.max - layerConfig.scaleRange.min);
    const alpha = layerConfig.alphaRange.min + _simpleRandom(noiseOffset + 2) * (layerConfig.alphaRange.max - layerConfig.alphaRange.min);
    ...
    const fragmentData: CloudFragmentData = {
      ...
      width: (sizeRange.w.min + _simpleRandom(noiseOffset + 4) * (sizeRange.w.max - sizeRange.w.min)) * scale,
      height: (sizeRange.h.min + _simpleRandom(noiseOffset + 5) * (sizeRange.h.max - sizeRange.h.min)) * scale,
      speed: (speedRange.min + _simpleRandom(noiseOffset + 6) * (speedRange.max - speedRange.min)) * layerConfig.speedMultiplier,
    ...
    const densityThreshold = fragmentData.type === 'dense' || fragmentData.type === 'puffy' ? 0.05 : 0.1;
        ...
        if (densityValue > densityThreshold) {
          ...
          if (distanceFromCenter <= organicBoundary) {
            ...
            const neighborDensities = [
              getMemoizedDensity(nx, ny - step), getMemoizedDensity(nx, ny + step),
              getMemoizedDensity(nx - step, ny), getMemoizedDensity(nx + step, ny)
            ];
            const avgNeighbor = neighborDensities.reduce((s, n) => s + n, 0) / neighborDensities.length;
            const isEdge = (fragmentData.type === 'dense' || fragmentData.type === 'puffy') ?
            ...
            const colorInfoForCalc = { density: densityValue, isEdge, edgeDistance, shadowFactor, brightness: currentBrightness };
            const calculatedColor = _calculatePixelColorLogic(colorInfoForCalc, initialSkyGradient || null);

            pixelProperties.push({
              nx, ny, density: densityValue, isEdge, edgeDistance, shadowFactor,
              brightness: currentBrightness, alpha: currentAlpha, color: calculatedColor,
  ...
  recalculatePixelColors(
  ...
  ): number[] { // Returns an array of new colors
    ...
    for (const prop of pixelProps) {
      ...
      newColors.push(_calculatePixelColorLogic(colorInfo, newSkyGradient));
    }
    return newColors;
  }
};
