...
export class SunCalculator {
  ...
  private determineCurrentTimePeriod(nowTs: number, sunTimes: ReturnType<typeof getSunTimes>, currentDate: Date): TimePeriodInfo {
    ...
    const todaySolarEvents: { name: TimeOfDay; time: number }[] = [
      ...
      { name: 'night_before_dawn' as TimeOfDay,   time: sunTimes.astronomicalDawn?.valid ? sunTimes.astronomicalDawn.ts : this.getStartOfDay(currentDate).getTime() }, // Approx
      ...
      { name: 'solar_noon_transition' as TimeOfDay, time: sunTimes.solarNoon.ts - 2 * 60 * 60 * 1000 },
      ...
      { name: 'evening_transition' as TimeOfDay,    time: sunTimes.sunsetStart.ts - 2 * 60 * 60 * 1000 },
      ...
      { name: 'deep_night' as TimeOfDay,            time: sunTimes.astronomicalDusk?.valid ? sunTimes.astronomicalDusk.ts : this.getEndOfDay(currentDate).getTime() }, // Approx
    ].sort((a, b) => a.time - b.time);

    // Add end of day event to cap the last period
    const endOfDayEvent = { name: 'deep_night' as TimeOfDay, time: this.getEndOfDay(currentDate).getTime() };
